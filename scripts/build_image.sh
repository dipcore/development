...
function get-configs()
{
    ...
    local TARGET_USERIMAGES_USE_UBIFS=$(get_build_var TARGET_USERIMAGES_USE_UBIFS)
    ...
    if [ "$TARGET_USERIMAGES_USE_UBIFS" == "true" ]; then
        INTERNAL_USERIMAGES_USE_UBIFS=true
        INTERNAL_USERIMAGES_FILE_SYSTEM_TYPE=ubifs
    fi
    ...
    MLC=$(cat $BOARDCONFIG_MK | grep 'MLC')
}

function mkubifs-extra-flags()
{
    local BOARD_NAND_BAD_BLOCK_PERCENTAGE=1
    local BOARD_NAND_PAGE_SIZE=$(get_build_var BOARD_NAND_PAGE_SIZE)
    if [ -z "$BOARD_NAND_PAGE_SIZE" ]; then
        BOARD_NAND_PAGE_SIZE=2048
    fi
    local BOARD_NAND_PEB_SIZE=$(get_build_var BOARD_NAND_PEB_SIZE)
    if [ -z "$BOARD_NAND_PEB_SIZE" ]; then
        BOARD_NAND_PEB_SIZE=131072
    fi

    local BOARD_NAND_LEB_SIZE=$(($BOARD_NAND_PEB_SIZE-$BOARD_NAND_PAGE_SIZE*2))
    local UBI_B7=$(($BOARD_NAND_PARTITION_SIZE/$BOARD_NAND_PEB_SIZE))
    local UBI_B8=$((($UBI_B7*$BOARD_NAND_BAD_BLOCK_PERCENTAGE-1)/100+1))
    local UBI_B9=$(($BOARD_NAND_PEB_SIZE-$BOARD_NAND_LEB_SIZE))
    local UBI_B10=$((($UBI_B8+4)*$BOARD_NAND_PEB_SIZE+$UBI_B9*($UBI_B7-$UBI_B8-4)))
    local UBI_B12=$(($UBI_B7-($UBI_B10+$BOARD_NAND_PEB_SIZE)/$BOARD_NAND_PEB_SIZE))
    local UBI_B13=$(($UBI_B12*$BOARD_NAND_PEB_SIZE/$BOARD_NAND_LEB_SIZE))

    mkubifs_extra_flags="-m $BOARD_NAND_PAGE_SIZE -e $BOARD_NAND_LEB_SIZE -c $UBI_B13"
}
...
function generate-userimage-prop-dictionary()
{
    ...
    if [ -n "$mkubifs_extra_flags" ]; then
        echo "mkubifs_extra_flags=$mkubifs_extra_flags" >> $1
    fi

    if [ "$HAVE_SELINUX" == "true" ]; then
...
function gernerate-script-start()
{
    ...
    [ -n "$MLC" ] && echo "mstar scripts/[[cis" >> $AUTO_UPDATE_SCRIPT
    echo "mstar scripts/set_partition" >> $AUTO_UPDATE_SCRIPT

    # Generate [[cis script
    if [ -n "$MLC" ]; then
        rm -rf $CIS_SCRIPT
        echo "tftp 0x50400000 NANDINFO.nni" >> $CIS_SCRIPT
        echo "tftp 0x50500000 PAIRPAGEMAP_v2.ppm" >> $CIS_SCRIPT
        echo "ncisbl 0x50400000 0x50500000" >> $CIS_SCRIPT
    fi
...
function gernerate-script-end()
{
    ...
    # Generate set_partition script
    if [ "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
        UBI_SIZE_16=0x$(echo "obase=16; $UBI_SIZE" | bc)

        echo "nand erase.chip" >> $SET_PARTITION_SCRIPT
        if [ -n "$MLC" ]; then
            echo "dynpart edb64M-nand:0xA00000(CIS),0x00100000(misc),$BOARD_RECOVERYIMAGE_PARTITION_SIZE(recovery),$BOARD_BOOTIMAGE_PARTITION_SIZE(boot),$UBI_SIZE_16(ubi),20m(TBL),20m(CTRL),-(reserved)" >> $SET_PARTITION_SCRIPT
        else
            echo "dynpart edb64M-nand:0x00100000(misc),$BOARD_RECOVERYIMAGE_PARTITION_SIZE(recovery),$BOARD_BOOTIMAGE_PARTITION_SIZE(boot),$UBI_SIZE_16(ubi),3m(TBL),3m(CTRL),-(reserved)" >> $SET_PARTITION_SCRIPT
        fi
        echo "saveenv" >> $SET_PARTITION_SCRIPT
    fi
}
...
function gernerate-raw-script()
{
    ...
    if [ "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
        if [ "$BUILD_WITH_SECURE_BOOT" == "true" ]; then
            ...
            echo "nand erase.part misc" >> $RECOVERY_SCRIPT
            echo "nand erase.part recovery" >> $RECOVERY_SCRIPT
            echo "nand write.e $BOARD_TFTP_ADDRESS recovery \$(filesize)" >> $RECOVERY_SCRIPT
            ...
            echo "setenv recoverycmd nand read.p $BOARD_BOOTM_BASE recovery $BOARD_RECOVERYIMAGE_PARTITION_SIZE\; bootm $BOARD_BOOTM_BASE" >> $RECOVERY_SCRIPT
            ...
            echo "nand erase.part recovery" >> $RECOVERY_SCRIPT
            echo "nand write.e $BOARD_TFTP_ADDRESS recovery \$(filesize)" >> $RECOVERY_SCRIPT
            echo "setenv recoverycmd nand read.e $BOARD_BOOTM_BASE recovery $BOARD_RECOVERYIMAGE_PARTITION_SIZE\; bootm $BOARD_BOOTM_BASE" >> $RECOVERY_SCRIPT
            echo "saveenv" >> $RECOVERY_SCRIPT
        fi

        if [ "$BUILD_WITH_SECURE_BOOT" == "true" ]; then
            echo "tftp $BOARD_TFTP_ADDRESS boot.img.aes" >> $BOOT_SCRIPT
            echo "nand erase.part boot" >> $BOOT_SCRIPT
            echo "nand write.e $BOARD_TFTP_ADDRESS boot \$(filesize)" >> $BOOT_SCRIPT
            ...
            echo "setenv LOAD_KERNEL nand read.p $BOARD_BOOTM_BASE boot $BOARD_BOOTIMAGE_PARTITION_SIZE\; authenticateAN $BOARD_BOOTM_BASE\;" >> $BOOT_SCRIPT
            ...
            echo "nand write.e $BOARD_TFTP_ADDRESS boot \$(filesize)" >> $BOOT_SCRIPT
            if [ "$BOARD_TEEIMAGE" == "true" ]; then
                echo "setenv LOAD_KERNEL nand read.e $BOARD_BOOTM_BASE boot $BOARD_BOOTIMAGE_PARTITION_SIZE" >> $BOOT_SCRIPT
                echo "setenv BOOT_KERNEL bootm $BOARD_BOOTM_BASE" >> $BOOT_SCRIPT
            else
                echo "setenv bootcmd nand read.e $BOARD_BOOTM_BASE boot $BOARD_BOOTIMAGE_PARTITION_SIZE\; bootm $BOARD_BOOTM_BASE" >> $BOOT_SCRIPT
            fi
            echo "saveenv" >> $BOOT_SCRIPT
        fi
        ...
        if [ "$BOARD_TEEIMAGE" == "true" ]; then
            ...
            if [ "$BUILD_WITH_SECURE_BOOT" == "true" ]; then
                ...
                echo "nand erase.part tee" >> $TEE_SCRIPT
                echo "nand write.p $BOARD_TFTP_ADDRESS tee \$(filesize)" >> $TEE_SCRIPT
                echo "setenv LOAD_NUTTX nand read.p 0x5EC70000 tee 0x600000" >> $TEE_SCRIPT
        ...
        if [ "$BOARD_RTPMIMAGE" == "true" ]; then
            ...
            echo "nand erase.p RTPM" >> $RTPM_SCRIPT
            echo "nand write.p $BOARD_TFTP_ADDRESS RTPM \$(filesize) 1" >> $RTPM_SCRIPT
        fi
    else
        if [ "$BUILD_WITH_SECURE_BOOT" == "true" ]; then
            echo "tftp $BOARD_TFTP_ADDRESS recovery.img.aes" >> $RECOVERY_SCRIPT
...
function gernerate-fs-script()
{
    ...
    if [ "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
        UBI_SIZE=$(($UBI_SIZE+$PARTITION_SIZE))

        echo "tftp $BOARD_TFTP_ADDRESS $PARTITION_NAME.img" >> $PARTITION_SCRIPT
        echo "ubi part ubi" >> $PARTITION_SCRIPT
        echo "ubi remove $PARTITION_NAME" >> $PARTITION_SCRIPT
        echo "ubi create $PARTITION_NAME $PARTITION_SIZE" >> $PARTITION_SCRIPT
        echo "ubi write $BOARD_TFTP_ADDRESS $PARTITION_NAME \$(filesize)" >> $PARTITION_SCRIPT
    else
...
function gernerate-fs-image()
{
    ...
    if [ "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
        BOARD_NAND_PARTITION_SIZE=$PARTITION_SIZE
        mkubifs-extra-flags
    fi
    if [ "$PARTITION_NAME" == "system" -o "$PARTITION_NAME" == "userdata" -o "$PARTITION_NAME" == "cache" -o "$PARTITION_NAME" == "systembackup" ]; then
        ...
        if [ "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
            mkfs.ubifs -F -r $TARGET_OUT -o $PRODUCT_OUT/$PARTITION_NAME.img $mkubifs_extra_flags
        else
...
function compress-image()
{
    ...
    if [ "$PARTITION_LZO" == "false" -o "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
        return
    fi
    ...
    do
        ...
        do
            ...
            if [ "$INTERNAL_USERIMAGES_USE_UBIFS" == "true" ]; then
                ...
                echo "ubi write_cont $BOARD_TFTP_ADDRESS $PARTITION_NAME $size_16" >> $PARTITION_SCRIPT
            else
                if [ "$PARTITION_LZO" == "true" ]; then
