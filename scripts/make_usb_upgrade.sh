...
#================================ Select Chip ==================================
read -p "Chip?  1)napoli 2)madison 3)kaiser 4)monaco 5)clippers 6)muji :" temp
if [ "$temp" == "1" ]; then
    CHIP=napoli
elif [ "$temp" == "2" ]; then
    CHIP=madison
...
#=============================== Select Board&Device ===========================
if [ "$CHIP" == "napoli" ]; then
    read -p "Device? 1)durian 2)lemon :" temp
    if [ "$temp" == "1" ]; then
        DEVICE=durian
    elif [ "$temp" == "2" ]; then
        DEVICE=lemon
    fi
elif [ "$CHIP" == "madison" ]; then
    read -p "Device? 1)mango 2)banana 3)grape :" temp
    if [ "$temp" == "1" ]; then
        DEVICE=mango
    elif [ "$temp" == "2" ]; then
        DEVICE=banana
...
elif [ "$CHIP" == "kaiser" ]; then
    DEVICE=melon
elif [ "$CHIP" == "monaco" ]; then
    ...
    read -p "Device? 1)orange 2)kiwi :" temp
    if [ "$temp" == "1" ]; then
        DEVICE=orange
    elif [ "$temp" == "2" ]; then
        DEVICE=kiwi
...
#=============================== Select Filesystem =============================
FS_FORMAT="EXT4"
#read -p "Storage? 1)EMMC; 2)NAND :" temp
#if [ "$temp" == "1" ]; then
#    FS_FORMAT="EXT4"
#elif [ "$temp" == "2" ]; then
#    FS_FORMAT="UBIFS"
#    NAND_FLASH_TYPE="MLC"
#    read -p "Nand flash type? 1)MLC; 2)SLC :" temp
#    if [ "$temp" == "1" ]; then
#        NAND_FLASH_TYPE="MLC"
#    elif [ "$temp" == "2" ]; then
#        NAND_FLASH_TYPE="SLC"
#    fi
#    printf "NAND_FLASH_TYPE: $NAND_FLASH_TYPE\n"
#fi
...
function func_init_common_env(){
    ...
    # image offset in MstarUpgrade.bin , we need this argument to load image
    # data from MstarUpgrade.bin when upgrade in UBoot
...
# write MAGIC_STRING to MstarUpgrade.bin
# write script file to MstarUpgrade.bin , and caculate crc value of script file
# write crc value to MstarUpgrade.bin
    ...
    if [ "$temp" == "Y" ] || [ "$temp" == "y" ]; then
        ...
        if [ "$FS_FORMAT" == "EXT4" ]; then
            ...
            if [ "$SPILT_FLAG" = "1" ]; then
                ...
                # write system.imgxx.lzo to MstarUpgrade.bin
    ...
    if [ "$temp" == "Y" ] || [ "$temp" == "y" ]; then
        ...
        if [ "$FS_FORMAT" == "EXT4" ]; then
            ...
            if [ "$SPILT_FLAG" = "1" ]; then
                ...
                # write userdata.imgxx.lzo to MstarUpgrade.bin
                for ((i=1; i<=$SPILT_NUM; i++))
                do
    ...
    if [ "$temp" == "Y" ] || [ "$temp" == "y" ]; then
        ...
        if [ "$FS_FORMAT" == "EXT4" ]; then
            ...
            if [ "$SPILT_FLAG" = "1" ]; then
                ...
                # write cache.imgxx.lzo to MstarUpgrade.bin
                for ((i=1; i<=$SPILT_NUM; i++))
                do
    ...
    if [ "$temp" == "Y" ] || [ "$temp" == "y" ]; then
        ...
        if [ "$FS_FORMAT" == "EXT4" ]; then
            ...
            if [ "$SPILT_FLAG" = "1" ]; then
                ...
                # write systembackup.imgxx.lzo to MstarUpgrade.bin
                for ((i=1; i<=$SPILT_NUM; i++))
                do
...
# upgrade partition .write commands of set_partition to MstarUpgrade.bin
